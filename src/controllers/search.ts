...
export const search = async (req: Request, res: Response) => {
    try {
        const { keyword, page: pageNum = 1 } = req.query;
        if (!keyword) {
            return res.status(400).json({ error: 'Keyword is required' });
        }

        const searchUrl = `${process.env.SCRAPE_WEB_BASE_URL}/?s=${encodeURIComponent(keyword as string)}&page=${pageNum}`;
        ...
        await page.setExtraHTTPHeaders({
            ...
            'Referer': process.env.SCRAPE_WEB_BASE_URL || 'https://hollymoviehd.cc',
            'Origin': process.env.SCRAPE_WEB_BASE_URL || 'https://hollymoviehd.cc',
        ...
        const cookies = [
            {
                name: 'cf_clearance',
                value: 'dummy',
                domain: new URL(process.env.SCRAPE_WEB_BASE_URL || 'https://hollymoviehd.cc').hostname,
                path: '/',
        ...
        await page.goto(process.env.SCRAPE_WEB_BASE_URL || 'https://hollymoviehd.cc', { waitUntil: 'networkidle0' });
